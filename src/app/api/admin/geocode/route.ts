// Admin address-lookup proxy — near-identical to
// src/app/api/studio/geocode/route.ts, gated on Admin's own session instead
// of Studio's.
//
...
// a staff member's keystrokes don't leave with the Admin page's Referer
// attached.
//
// Session-gated on purpose: this is an Admin tool, not a public endpoint —
// without the check, anyone could point a script at it and burn the shared
// (rate-limited, unauthenticated) upstream quota that every real user
// depends on. requireAdminSession() redirects rather than throws, so this
// uses getAdminSession() and returns a plain 401 instead: a fetch() from
// the form wants a status code, not an HTML redirect to /admin/login.
