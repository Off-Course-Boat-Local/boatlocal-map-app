...
export default function StatCard({ label, value, hint }: StatCardProps) {
  return (
    <div className="flex h-full flex-col rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-[var(--admin-shadow-card)] transition-shadow hover:shadow-[var(--admin-shadow-float)]">
      <p className="text-[0.6875rem] font-semibold tracking-[0.14em] text-[var(--admin-ink-soft)] uppercase">
        {label}
      </p>
      <p className="font-display mt-auto pt-3 text-[2.25rem] leading-none font-semibold tracking-tight text-[var(--admin-ink)]">
        {value}
      </p>
      {hint ? <p className="mt-2.5 text-xs text-[var(--admin-ink-soft)]">{hint}</p> : null}
    </div>
  );
}
