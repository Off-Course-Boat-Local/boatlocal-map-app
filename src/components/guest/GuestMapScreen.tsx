...
export default function GuestMapScreen({
  ...
  // onAction/onSecondaryAction already hand back the specific card's OWN
  // item, so this resolves the full MapPin by id rather than closing over
  // `selected` — in a carousel that's not necessarily the centred card.
  function handlePlaceAction(item: { id: string }) {
    ...
    // i want to build something internal".
    startDirections(pin, "walk");
  }

  // Same screen, transit itinerary — founder request, 2026-09-04. Boats
  // never reach this (PlaceCard hides the button for them).
  function handlePlaceSecondaryAction(item: { id: string }) {
    const pin = allPins.find((p) => p.id === item.id);
    if (!pin) return;
    startDirections(pin, "transit");
  }
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 flex flex-col items-center">
          ...
          {selectedCluster && selectedCluster.length > 1 ? (
            <PlaceCardCarousel
              ...
              onSecondaryAction={handlePlaceSecondaryAction}
              className="w-full"
            />
          ) : (
            <PlaceCard
