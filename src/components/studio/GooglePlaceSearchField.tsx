...
// "Search Google Maps" enrichment for Studio's RecommendationForm — searches
...
// fields. Same mechanics as src/components/admin/GooglePlaceSearchField.tsx,
// styled with Studio's own theme tokens and hitting the Studio-gated
// /api/studio/places/* routes instead of Admin's.
//
// This is a deliberate, scoped override of the house rule documented in
// src/lib/studio/geocode.ts — see googlePlaces.ts's header and
// src/lib/admin/adminRecommendationForm.ts's "NOTE ON GOOGLE PLACES" comment
// for the full rationale, now extended to cover a company's own
// recommendations too (founder request, 2026-09-01). The Address field below
// this one still searches Photon/OSM — nothing else about that changes.
...
export default function GooglePlaceSearchField({ query, onApply }: GooglePlaceSearchFieldProps) {
  ...
  async function pick(placeId: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/studio/places/details?placeId=${encodeURIComponent(placeId)}`);
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className={`${inputClass} flex w-full items-center justify-center gap-2 !bg-[var(--studio-bg)] font-medium hover:opacity-90 disabled:opacity-60`}
      >
          ...
          {results.map((r) => (
            <li key={r.placeId}>
              <button
                ...
                className="block w-full px-3 py-2 text-left hover:bg-[var(--studio-bg)] disabled:opacity-60"
              >
      ...
      {open && !searching && results.length === 0 && !error ? (
        <p className="text-xs text-[var(--studio-ink-soft)]">No matches near Amsterdam.</p>
      ) : null}

      {applying ? (
        <p className="text-xs text-[var(--studio-ink-soft)]">Pulling hours, category, and photos…</p>
      ) : null}
    </div>
  );
}
